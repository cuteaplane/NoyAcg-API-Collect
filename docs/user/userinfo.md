...
用于获取当前登录账户的详细个人资料、资产状态（积分）以及佩戴的勋章列表。此接口还用于在执行自动化任务前检查 `NOY_SESSION` 的存活状态。
...
这份文档已经完全去掉了多余的路径符号，cuteaplane 直接复制到 GitHub 的文件里就好啦！(๑>◡<๑) 所有的 `status` 无论是字符串 `"ok"` 还是数字 `200` 都记录得清清楚楚喵！
